...
class Template(BaseModel):
    ...
    def inherit(self, base: "Template") -> "Template":
        styles = {name: {**base.theme.text_styles.get(name, {}), **self.theme.text_styles.get(name, {})} for name in set(base.theme.text_styles) | set(self.theme.text_styles)}
        theme = Theme(
    ...
    def _resolve_variant(self, name: str, seen: set[str]) -> TemplateVariant:
        if name in seen:
            raise ValueError(f"Circular template variant inheritance involving {name!r}")
        try:
        ...
        if not variant.extends:
            if variant.profile is None or variant.root is None:
                raise ValueError(f"Variant {name!r} must define profile and root or extend another variant")
        ...
        updates["required_slots"] = list(dict.fromkeys([*base.required_slots, *variant.required_slots]))
        updates["max_chars_by_slot"] = {**base.max_chars_by_slot, **variant.max_chars_by_slot}
    ...
    def select_variant(self, content: Mapping[str, Any], variant: Optional[str] = None) -> tuple[str, TemplateVariant]:
        if variant:
            return variant, self.resolve_variant(variant)
        candidates = []
        for name in self.variants:
            item = self.resolve_variant(name)
            if any(not lookup(content, slot) for slot in item.required_slots):
                continue
            overflow = sum(max(0, len(str(lookup(content, slot, ""))) - maximum) for slot, maximum in item.max_chars_by_slot.items())
    ...
    def build(self, content: Mapping[str, Any], variant: Optional[str] = None, **overrides: Any) -> PostConfig:
        from .presets.platforms import get_profile

        variant_name, selected = self.select_variant(content, variant)
        assert selected.profile and selected.root
        profile = get_profile(selected.profile)
        canvas = Box(0, 0, profile.width, profile.height)
        safe = Box(profile.safe_area.left, profile.safe_area.top, profile.width - profile.safe_area.left - profile.safe_area.right, profile.height - profile.safe_area.top - profile.safe_area.bottom)
        root_box = canvas if selected.root_scope == "canvas" else safe
        texts, images, shapes = resolve_layout(selected.root, root_box, content, self.theme, canvas, safe)
        config: Dict[str, Any] = {
            ...
            "layout_policy": selected.layout_policy or LayoutPolicyConfig(),
            "texts": texts, "images": images, "shapes": shapes,
            "meta": {"title": f"{self.name}:{variant_name}"},
        }
        config.update(overrides)
        return PostConfig(**config)

    def render(self, content: Mapping[str, Any], variant: Optional[str] = None, **overrides: Any) -> TemplateRenderResult:
        from .api import render

        post = self.build(content, variant, **overrides)
        result = render(post, save=False)
        return TemplateRenderResult(post=post, images=result.images, reports=result.reports)

    def render_fixtures(self) -> Dict[str, TemplateRenderResult]:
        return {name: self.render(fixture.content, fixture.variant) for name, fixture in self.fixtures.items()}


__all__ = ["LayoutNode", "PreviewFixture", "Template", "TemplateRenderResult", "TemplateVariant", "Theme"]
